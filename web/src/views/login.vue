<template>
  <a-row class="login">
    <a-col :span="8" :offset="8" class="login-main">
      <h1 style="text-align: center"><rocket-two-tone />&nbsp;彰广12306售票系统</h1>
      <a-form
          :model="loginForm"
          name="basic"
          autocomplete="off"
...
export default defineComponent({
  ...
  setup() {
    ...
    const login = () => {
      axios.post("http://localhost:8000/member/member/login", loginForm).then(response => {console.log(response);
        let data = response.data;
        if (data.success){
          notification.success({ description:'登陆成功！'})
          console.log("登陆成功：", data.message)
        }else {
          notification.error({ description: data.message});
        }
      })
